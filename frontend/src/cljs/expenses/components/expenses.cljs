...
(defn plain-expenses-to-formatted [raw]
  (let [rows (.split raw "\n")]
    (into [] (map (fn [row]
                    (let [
                          v (parse-expense row)
                          name (string/trim (first v))
                          amount (-> v second string/trim js/parseFloat)
                          ]
...
(defn format-expense-to-plain [exps]
  (let [
        space 5
        longest (+ (apply max (into [] (map #(count (string/trim  (:name %))) exps))) space)
        filler (fn [s] (apply str (repeat (- longest (count s)) " ")))]
    (string/join "\n"
                         (map #(string/join (filler (:name %)) (vals %)) exps))))
